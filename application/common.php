...
if (!function_exists('imgUrl')) {
    function imgUrl($data, $field) {
        $data[$field] = cdnurl($data[$field], true);
        return $data;
    }
}

if (!function_exists('imgsurl')) {
    /**
     * 获取上传资源的CDN的地址
     * @param string  $url    资源相对地址
     * @param boolean $field  字段名称
     * @return string
     */
    function imgsurl($data, $field){
        if(is_array($data)){
            foreach ($data as $ke=>$v) {
                $data[$ke][$field] = cdnurl($v[$field],true);
