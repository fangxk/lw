...
class Cart extends Api
{
    ...
    {
        $list = model('\app\api\model\wanlshop\Cart')
            ->where('user_id', $this->auth->id)
            ->field('shop_id')
            ->group('shop_id')
            ->select();
        $list = collection($list)->each(function ($item) {
            $shop =  Db("wanlshop_shop")->field("shopname,id")->where("id={$item['shop_id']}")->find();
            $item["shop_name"] =$shop["shopname"];
            $item['checked'] = false;
            $item['goods'] = model('\app\api\model\wanlshop\Cart')->with('goods')
                ->where(['cart.user_id' => $this->auth->id, 'cart.shop_id' => $item['shop_id']])
                ->select();
            return $item;
        });
		...
		if ($this->request->isPost()) {
			...
			// 新增购物车
			    ...
			    if($cart){
    				...
    				$params = [
    					'number' => $number,
    					'sum' => bcmul($cart['suk']['price'], $number)
    				];
    				$cart->save($params);
				}else{
					// 只新增ID，1.0.2升级
    ...
    public function updateCart()
    {
        ...
        $cart = $this->model->where('id', $post['id'])->find();
        $sum = bcmul($cart['suk']['price'], $post['number']);
        $cart->save(['number' => $post['number'], 'sum' => $sum]);
        $this->success('更新购物车完成！');
    }
