...
class Product extends Api
{
    ...
    public function lists()
    {
    	...
    	$this->success('返回成功', ($list->toArray()));
    }
    ...
    public function goods()
    {
		...
		$goods = $goodsModel
			->where(['id' => $id])
			->field('id,category_id,shop_category_id,brand_id,freight_id,shop_id,title,image,images,flag,content,category_attribute,activity,price,sales,payment,comment,praise,moderate,negative,like,views,status')
			->find();
		// 浏览+1 & 报错
	...
	{
	    $pagesize = $this->request->param("pagesize")?$this->request->param("pagesize"):config('paginate.list_rows');
