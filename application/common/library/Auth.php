...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username', 'nickname', 'mobile', 'avatar', 'score','roles','virtual_num','real_num',"experience_num"];
 ...
 /*   public function register($username, $password, $email = '', $mobile = '', $extend = [])
    {
        ...
        $params = array_merge($data, [
            'nickname'  => $username,//preg_match("/^1[3-9]{1}\d{9}$/",$username) ? substr_replace($username,'****',3,4) : $username,
            'salt'      => Random::alnum(),
            'jointime'  => $time,
            'joinip'    => $ip,
            'logintime' => $time,
     ...
     * @return boolean
     */
    public function direct1($user_id)
    {
        $user = User::get($user_id);
        if ($user) {
            Db::startTrans();
